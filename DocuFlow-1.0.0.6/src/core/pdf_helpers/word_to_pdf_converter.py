...
try:
    ...
    print("WARNING: PyPDF2 not installed. PDF validation and metadata will be unavailable.")
...
try:
    ...
    print("WARNING: python-docx not installed. Word validation will be limited.")
...
try:
    ...
    print("WARNING: path_utils not available. Will use basic copy method.")
...
def _fallback_safe_copy(source_path: Path, temp_dir: Path) -> Optional[Path]:
    ...
    try:
        ...
        print(f"ERROR: Failed to create safe copy: {e}")
        return None
...
class WordToPdfConverter:
    ...
    def _apply_pdf_metadata(pdf_path: Path, metadata: Dict[str, str]) -> bool:
        ...
        if not PYPDF2_AVAILABLE:
            print("WARNING: PyPDF2 not available. Skipping metadata application.")
            return False
    ...
    def _perform_conversion(
        ...
        if task.metadata:
            ...
            if not metadata_success:
                print(f"WARNING: Metadata application failed for {task.filename}")
...
def print_report(report: Report):
    ...
    print(f"  Speed:          {report.summary['files_per_second']} files/second")
    print(f"  Workers used:   {report.summary['workers_used']}")
    
    if report.errors:
        print(f"\nErrors ({len(report.errors)}):")
        for error in report.errors[:5]:  # Show first 5
            print(f"  - {error['task'].source.name}: {error['message']}")
...
if __name__ == "__main__":
    ...
    config = Config(
        ...
        callbacks=CallbackConfig(
            on_progress=lambda current, total, name: print(f"Progress: {current}/{total} - {name}"),
            on_error=lambda task, error, retries: print(f"Error on {task.source.name}: {error}"),
            on_complete=lambda report: print("Conversion complete!")
        )
    )
