...
try:
    ...
    print("="*80)
    print("❌ ERROR CRÍTICO (PDF_Converter): La biblioteca 'PyMuPDF' no está instalada.")
    print("   Por favor, ejecute: pip install PyMuPDF")
    print("="*80)
    fitz = None
...
def _parse_page_ranges(ranges_str: Optional[str], max_pages: int) -> List[int]:
    ...
    try:
        ...
        print(f"⚠️ Error parseando rango de página '{ranges_str}': {e}. Usando todas las páginas.")
        return list(range(max_pages))
