...
try:
    ...
    from core.pdf_helpers.pdf_converter import convert_pdf_to_images
    
except ImportError as e:
    print(f"❌ CRITICAL (DocumentAssembler): No se pudieron importar módulos del Core.")
    print(f"   Error: {e}")
    sys.exit(1)
...
def _neuter_trailing_empty_paragraph(doc: DocxDocument):
    ...
    try:
        ...
        print(f"⚠️ Advertencia: Falló al neutralizar párrafo fantasma: {e}")
...
def assemble_document_with_media(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if debug: print(f"--- [INICIO TRABAJO ENSAMBLADOR (DEBUG)] ---")
    ...
    try:
        ...
        if pdf_map:
            if debug: print(f"  [ASM] Detectados {len(pdf_map)} trabajos PDF. Iniciando pre-procesamiento.")
            ...
            with create_safe_temp_copy(source_path) as temp_doc_path:
                ...
                for pdf_job in pdf_map:
                    ...
                    try:
                        ...
                        if debug: print(f"    [ASM] Procesando PDF: '{placeholder}'")
                        ...
                        if debug: print(f"    [ASM] ✅ PDF '{placeholder}' expandido en {len(new_img_jobs)} imágenes.")

                    except Exception as e:
                        pdf_preprocessing_report["failed"].append({"placeholder": placeholder, "error": str(e)})
                        if debug: print(f"    [ASM] ❌ FALLO PDF '{placeholder}': {e}")
                        raise
                ...
                if config.get("remove_trailing_blank_page", False):
                    if debug: print("  [ASM] Aplicando neutralización de página en blanco...")
                ...
                if debug: print(f"  [ASM] Pre-procesamiento PDF completado. Documento expandido guardado.")
        ...
        if debug: 
            print(f"  [ASM] 🔥 ERROR CRÍTICO en pre-procesamiento PDF: {pdf_error}")
            print(f"  [ASM] REVIRTIENDO a modo 'solo-imágenes'.")
    ...
    try:
        if debug:
            print(f"  [ASM] Iniciando Etapa 2: Inserción Final.")
            print(f"    [ASM] Fuente: {source_for_inserter}")
            print(f"    [ASM] Destino: {config.get('target_path')}")
            print(f"    [ASM] Total trabajos de imagen: {len(final_image_map)}")
        ...
        if debug: print(f"  [ASM] Inserción final completada.")
        
    except Exception as e:
        if debug: print(f"  [ASM] ❌ ERROR CRÍTICO en Etapa 2 (Inserción): {e}")
        final_report = {
            "success": False,
        ...
        if os.path.exists(main_temp_dir):
            try:
                ...
                if debug: print(f"  [ASM] Directorio temporal principal eliminado.")
            except OSError as e:
                if debug: print(f"  [ASM] ⚠️ No se pudo eliminar el temp dir: {e}")

    if debug: print(f"--- [FIN TRABAJO ENSAMBLADOR (DEBUG)] ---")
