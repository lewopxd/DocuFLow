...
def _build_regex_map(definitions: List[Dict[str, str]]) -> Dict[str, re.Pattern]:
    ...
    for definition in definitions:
        try:
            ...
            # --- [ INICIO DE CORRECCIÓN (v2) ] ---
            # El bug estaba en la regex anterior: [^\s]+?
            # Esa regex no permitía espacios (ej. "$PDF{{PDF HV}}")
            #
            # La nueva regex: .+?
            # Significa:
            # .      -> "Cualquier carácter (incluyendo espacios)"
            # +?     -> "Una o más veces, de forma no-codiciosa (non-greedy)"
            #
            # Esto permite "$PDF{{PDF HV}}" y
            # "$IMG{{Anexo: Pantallazo...}}" sin problemas.
            
            pattern_str = f"({prefix}.+?{suffix})"
            # --- [ FIN DE CORRECCIÓN (v2) ] ---
