...
try:
    ...
    print(f"❌ CRITICAL: No se pudo importar 'core.file_helpers.storage.AppStorage'.")
    print(f"   Error: {e}")
...
def start_webview_app(handshake_event: multiprocessing.Event, config: dict):
    ...
    try:
        ...
        print(f"❌ CRITICAL (Hijo): No se pudo importar 'core.file_helpers.storage.AppStorage'.")
        print(f"   Error: {e}")
        AppStorage = None
    ...
    try:
        ...
        print(f"❌ CRITICAL (Hijo): No se pudo importar 'core.data_sheet.data_sheet_parser.get_data_sheet_structure'.")
        print(f"   Error: {e}")
    ...
    class BridgeAPI:
        ...
        def _handle_get_ui_settings(self, content: dict) -> dict:
            ...
            try:
                ...
                if not LOAD_SAVED_SESSION:
                    print("✓ Desactivación de sesión: Purgando 'last_opened_sheet' y 'last_opened_template' antes de enviar a la UI.")
        ...
        def _handle_get_excel_structure(self, content: dict) -> Optional[Dict[str, Any]]:
            ...
            print(f"✓ Bridge: Solicitando estructura para: {file_path}")
            
            structure_data = get_data_sheet_structure(file_path)
            
            if structure_data is None:
                print(f"⚠️ Bridge: El parser devolvió None para: {file_path}")
